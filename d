...
local function applyWeaponVisual(itemId)
	...
	print(("[Carti Hub] Applied visual for %s to %s"):format(tostring(itemId), refName))
	return true
end
...
local function spawnWeaponByIdNoPopup(itemId, itemType, amount)
	...
	local displayName = itemData.ItemName or itemData.Name or itemId
	print(("[Carti Hub] Spawned %dx %s without popup (%s/%s)"):format(amount, displayName, itemType, itemId))
	return true
end
...
local function spawnAllAncientWeapons(amount)
	...
	print(("[Carti Hub] Spawned %dx of %d ancient weapons"):format(amount, spawnedCount))
	return spawnedCount
end
...
local function isChromaWeapon(itemId, data)
	...
	or data.ChromaTexture ~= nil
	or data.ChromaStaticLayer ~= nil then
		return true
	end
...
local function spawnAllChromaWeapons(amount)
	...
	print(("[Carti Hub] Spawned %dx of %d chroma weapons"):format(amount, spawnedCount))
	return spawnedCount
end
...
local function isOfferSpawnerWeapon(data)
	...
	return itemType == "knife" or itemType == "sword"
end

local addSpecificItemToTheirOffer
local removeLastTheirOffer

local UpgradePlayerDropdown
local AutoSelectLastTradedToggle
local AutoFriendJoinToggle
...
local SpawnerFrame = Window:CreateTab("Spawner")
local TradeFrame = Window:CreateTab("Trade")
UpgradingFrame = Window:CreateTab("Upgrading")
local SettingsFrame = Window:CreateTab("Settings")
local KeybindsFrame = Window:CreateTab("Keys")

-- [weapon/offer/realistic spawner grid UI removed; rebuilt with Rayfield dropdowns near end of file]
-- [Spawner/Trade tab controls (specific weapon input, bulk-spawn buttons, weapon browser,
-- realistic spawner, offer spawner, start trade) are built with Rayfield near the end of the
-- file, once every handler function below is defined.]
...
function CartiHubSetSelectedUpgradePlayer(username)
	...
	if UpgradePlayerDropdown and username then
		UpgradePlayerDropdown:Set({ username })
	end
end

function CartiHubRefreshUpgradePlayerDropdown()
	if not UpgradePlayerDropdown then
		return
	end
	...
	UpgradePlayerDropdown:Refresh(names, true)

	if not selectedStillHere then
		CartiHubSetSelectedUpgradePlayer(nil)
	elseif CartiHubSelectedUpgradePlayerName then
		UpgradePlayerDropdown:Set({ CartiHubSelectedUpgradePlayerName })
	end
end

CartiHubAutoSelectLastTradedEnabled = false

function CartiHubSetAutoSelectLastTraded(enabled)
	CartiHubAutoSelectLastTradedEnabled = enabled == true
	if AutoSelectLastTradedToggle then
		AutoSelectLastTradedToggle:Set(CartiHubAutoSelectLastTradedEnabled)
	end
...
_G.CartiHubDebugFakeTradeAccept = true
_G.CartiHubLastAcceptTrace = nil
warn("[Carti Hub][AcceptDebug] enabled")
...
function CartiHubUpdateLastTradedPlayerFromGui()
	...
	for _, descendant in ipairs(tradeGui:GetDescendants()) do
		if descendant:IsA("TextLabel") or descendant:IsA("TextButton") or descendant:IsA("TextBox") then
			local playerName = CartiHubGetPlayerNameFromTradeText(descendant.Text)
...
function CartiHubApplyPendingRequestUsername(moduleSenderName)
	local pendingName = _G.CartiHubPendingFakeTradeUsername
	local gui = tradeModule.GUI
	local requestFrame = gui and gui.RequestFrame

	if not pendingName or not moduleSenderName or not requestFrame then
		return
	end

	for _, descendant in ipairs(requestFrame:GetDescendants()) do
		if descendant:IsA("TextLabel") or descendant:IsA("TextButton") or descendant:IsA("TextBox") then
			local text = tostring(descendant.Text or "")
			if text:find(tostring(moduleSenderName), 1, true) then
				descendant.Text = text:gsub(tostring(moduleSenderName), tostring(pendingName))
			end
		end
	end
end
...
local function setAutoFriendJoinEnabled(enabled)
	...
	if AutoFriendJoinToggle then
		AutoFriendJoinToggle:Set(enabled)
	end
...
function CartiHubEnsureFakeTradeWatermark()
	local tradeGui = tradeModule.GUI and tradeModule.GUI.TradeGUI
	local parent = tradeGui and (tradeGui:FindFirstChild("Container") or tradeGui)
	if not parent then
		return
	end

	local watermark = parent:FindFirstChild("CartiHubFakeTradeWatermark")
	if not watermark then
		watermark = Instance.new("TextLabel")
		watermark.Name = "CartiHubFakeTradeWatermark"
		watermark.BackgroundTransparency = 1
		watermark.BorderSizePixel = 0
		watermark.AnchorPoint = Vector2.new(1, 0)
		watermark.Size = UDim2.new(0, 120, 0, 16)
		watermark.Position = UDim2.new(1, -8, 0, 6)
		watermark.Text = "t.me/cartiscripts"
	...
	watermark.Visible = _G.CartiHubFakeTradeActive == true
	watermark.ZIndex = 10000
end
...
function CartiHubEnforceFakeTradeFrame()
	...
	CartiHubEnsureFakeTradeWatermark()

	if tradeModule.GUI.RequestFrame then
...
function CartiHubConnectFakeTradeFrameGuardSignals()
	...
	if requestFrame then
		table.insert(_G.CartiHubFakeTradeGuardConnections, requestFrame:GetPropertyChangedSignal("Visible"):Connect(CartiHubQueueFakeTradeFrameRepair))
	end
end
...
function CartiHubTraceFakeAcceptState(reason)
	if not _G.CartiHubDebugFakeTradeAccept then
		return
	end

	local actions = tradeModule.GUI and tradeModule.GUI.Actions
	local accept = actions and actions:FindFirstChild("Accept")
	local actionButton = accept and accept:FindFirstChild("ActionButton")
	local cooldown = accept and accept:FindFirstChild("Cooldown")
	local confirm = accept and accept:FindFirstChild("Confirm")
	local cancel = accept and accept:FindFirstChild("Cancel")
	local addItem = accept and accept:FindFirstChild("AddItem")
	local containerOverlay = accept and accept:FindFirstChild("ClientPlaceholderTradeActionOverlay")
	local actionOverlay = actionButton and actionButton:FindFirstChild("ClientPlaceholderTradeActionOverlay")
	local color = actionButton and actionButton:IsA("GuiObject") and actionButton.BackgroundColor3
	local colorText = color and ("%d,%d,%d"):format(color.R * 255, color.G * 255, color.B * 255) or "nil"

	local signature = table.concat({
		tostring(reason),
		tostring(localAcceptMode),
		tostring(_G.CartiHubFakeTradeActive),
		tostring(accept and accept.Visible),
		tostring(actionButton and actionButton.Visible),
		tostring(actionButton and actionButton:IsA("GuiButton") and actionButton.Active),
		tostring(actionButton and actionButton:IsA("GuiButton") and actionButton.AutoButtonColor),
		tostring(actionButton and actionButton:IsA("GuiObject") and actionButton.BackgroundTransparency),
		colorText,
		tostring(cooldown and cooldown.Visible),
		tostring(confirm and confirm.Visible),
		tostring(cancel and cancel.Visible),
		tostring(addItem and addItem.Visible),
		tostring(containerOverlay and containerOverlay.Visible),
		tostring(actionOverlay and actionOverlay.Visible),
	}, "|")

	if signature == _G.CartiHubLastAcceptTrace then
		return
	end

	_G.CartiHubLastAcceptTrace = signature
	warn("[Carti Hub][AcceptDebug] " .. signature)
end
...
local function scheduleOtherSideAccept()
	...
	task.delay(delaySeconds, function()
		...
		task.delay(1, function()
			...
			CartiHubCleanupRequestAcceptBridge()
		end)
	end)
end
...
local function addItemToTheirOfferStack(itemId, itemType, amount)
	amount = tonumber(amount) or 1
...
addSpecificItemToTheirOffer = function(itemId, itemType)
	if not tradeModule.GUI.TradeGUI.Enabled then
	...
	if not addItemToTheirOfferStack(itemId, itemType, 1) then
		warn("[IncomingTradePopup] Their offer is full.")
		return
	end
	...
	print(("[IncomingTradePopup] Added offer-spawner item: %s/%s"):format(tostring(itemType), tostring(itemId)))
end
...
local function getAcceptButton()
	local gui = tradeModule.GUI
	local requestFrame = gui and gui.RequestFrame
	local receivingRequest = requestFrame and requestFrame:WaitForChild("ReceivingRequest")
	return receivingRequest and receivingRequest:WaitForChild("Accept")
end

cleanupOldOverlays()

function CartiHubCleanupRequestAcceptBridge()
	if requestAcceptConnection then
		requestAcceptConnection:Disconnect()
		requestAcceptConnection = nil
	end

	local acceptButton = getAcceptButton()
...
installRequestAcceptBridge = function()
	...
	local acceptButton = getAcceptButton()
	if not acceptButton then
		warn("[IncomingTradePopup] Accept button missing.")
		return
	end
	...
	local function openOnce()
		if opened or not bridge.Parent then
			return
		end
	...
	task.delay(20, function()
		...
		if bridge.Parent or overlay.Parent then
			CartiHubCleanupRequestAcceptBridge()
		end
	end)
end
...
showIncomingTradeRequest = function()
	local moduleSenderName = pickFakeTradeSenderName()
	installRequestAcceptBridge()
	tradeModule.UpdateTradeRequestWindow("ReceivingRequest", {
		Sender = {
			Name = moduleSenderName,
		},
	})

	if _G.CartiHubPendingFakeTradeUsername then
		CartiHubApplyPendingRequestUsername(moduleSenderName)
		task.defer(function()
			CartiHubApplyPendingRequestUsername(moduleSenderName)
		end)
...
-- Rayfield UI: built here (rather than near the top) because it wires directly into the
-- handler functions/locals defined above (showIncomingTradeRequest, addRandomGodlyToTheirOffer,
-- removeLastTheirOffer, applyAvatarFromUserId, setAutoFriendJoinEnabled, CartiHub* upgrade
-- helpers). Game logic above is untouched; this section only creates the menu.

local function collectSpawnerWeapons(filterFn)
	local weapons = {}
	local seen = {}

	local function collect(container, itemType)
		for itemId, data in pairs(container or {}) do
			if type(data) == "table" and filterFn(itemId, itemType, data) and not seen[itemId] then
				seen[itemId] = true
...
local function filterNames(names, query)
...
local spawnerWeaponList = collectSpawnerWeapons(function(_, _, data)
	return isSpawnerRarity(data)
end)
local spawnerWeaponByName = {}
local spawnerWeaponNames = {}
for _, weapon in ipairs(spawnerWeaponList) do
	spawnerWeaponByName[weapon.name] = weapon
	table.insert(spawnerWeaponNames, weapon.name)
end

local offerWeaponList = collectSpawnerWeapons(function(itemId, itemType, data)
	return isOfferSpawnerWeapon(data) and canFakeTradeItem(itemId, itemType)
end)
local offerWeaponByName = {}
local offerWeaponNames = {}
for _, weapon in ipairs(offerWeaponList) do
	offerWeaponByName[weapon.name] = weapon
	table.insert(offerWeaponNames, weapon.name)
end

local WeaponBrowserDropdown
local RealisticDropdown
local OfferDropdown

-- Spawner tab

SpawnerFrame:CreateSection("Specific Weapon")

local specificWeaponName = ""

SpawnerFrame:CreateInput({
	Name = "Weapon Name",
	PlaceholderText = "e.g. Harvester",
	RemoveTextAfterFocusLost = false,
	Callback = function(value)
		specificWeaponName = value
	end,
})

SpawnerFrame:CreateSlider({
	Name = "Weapon Amount",
	Range = { 1, MAX_WEAPON_AMOUNT },
	Increment = 1,
	CurrentValue = 1,
	Callback = function(value)
...
local SpawnWeaponBtn = SpawnerFrame:CreateButton({
	Name = "Spawn Weapon",
	Callback = function()
		if specificWeaponName and specificWeaponName ~= "" then
			local success = spawnWeapon(specificWeaponName, currentWeaponAmount)
			SpawnWeaponBtn:Set(success and "Spawned" or "Not Found")
		else
			SpawnWeaponBtn:Set("Type A Weapon Name First")
		end

		task.delay(2, function()
			SpawnWeaponBtn:Set("Spawn Weapon")
		end)
	end,
})

SpawnerFrame:CreateSection("Bulk Spawn")

local SpawnGodliesBtn = SpawnerFrame:CreateButton({
	Name = "Spawn All Godlies",
	Callback = function()
		local count = spawnAllGodlyWeapons(currentWeaponAmount)
		if count > 0 then
			SpawnGodliesBtn:Set("Spawned " .. count .. " Godlies")
			task.delay(2, function()
				SpawnGodliesBtn:Set("Spawn All Godlies")
			end)
		end
	end,
})

local SpawnChromasBtn = SpawnerFrame:CreateButton({
	Name = "Spawn All Chromas",
	Callback = function()
		local count = spawnAllChromaWeapons(currentWeaponAmount)
		SpawnChromasBtn:Set(count > 0 and ("Spawned " .. count .. " Chromas") or "No Chromas Found")
		task.delay(2, function()
			SpawnChromasBtn:Set("Spawn All Chromas")
		end)
	end,
})

local SpawnAncientsBtn = SpawnerFrame:CreateButton({
	Name = "Spawn All Ancients",
	Callback = function()
		local count = spawnAllAncientWeapons(currentWeaponAmount)
		SpawnAncientsBtn:Set(count > 0 and ("Spawned " .. count .. " Ancients") or "No Ancients Found")
		task.delay(2, function()
			SpawnAncientsBtn:Set("Spawn All Ancients")
		end)
	end,
})

SpawnerFrame:CreateSection("Weapon Browser")

local selectedBrowserWeaponName = spawnerWeaponNames[1]
...
SpawnerFrame:CreateInput({
	...
	Callback = function(query)
		local filtered = filterNames(spawnerWeaponNames, query)
		WeaponBrowserDropdown:Refresh(filtered, true)
		selectedBrowserWeaponName = filtered[1]
	end,
})

WeaponBrowserDropdown = SpawnerFrame:CreateDropdown({
	Name = "Weapon",
	Options = spawnerWeaponNames,
	CurrentOption = { spawnerWeaponNames[1] },
	MultipleOptions = false,
	Callback = function(value)
		selectedBrowserWeaponName = type(value) == "table" and value[1] or value
	end,
})

SpawnerFrame:CreateButton({
	Name = "Spawn Selected",
	Callback = function()
		local weapon = selectedBrowserWeaponName and spawnerWeaponByName[selectedBrowserWeaponName]
		if weapon then
			spawnWeaponById(weapon.itemId, weapon.itemType, currentWeaponAmount)
		end
	end,
})

SpawnerFrame:CreateSection("Realistic Spawner")

local selectedRealisticNames = {}
local realisticMinAmount = 1
local realisticMaxAmount = 10
...
SpawnerFrame:CreateInput({
	...
	RemoveTextAfterFocusLost = false,
	Callback = function(query)
		RealisticDropdown:Refresh(filterNames(spawnerWeaponNames, query), true)
	end,
})

RealisticDropdown = SpawnerFrame:CreateDropdown({
	Name = "Weapons",
	Options = spawnerWeaponNames,
	CurrentOption = {},
	MultipleOptions = true,
	Callback = function(value)
		selectedRealisticNames = type(value) == "table" and value or { value }
	end,
})
...
SpawnerFrame:CreateInput({
	...
	RemoveTextAfterFocusLost = false,
	Callback = function(value)
		realisticMinAmount = math.clamp(math.floor(tonumber(value) or 1), 1, MAX_WEAPON_AMOUNT)
	end,
})
...
SpawnerFrame:CreateInput({
	...
	RemoveTextAfterFocusLost = false,
	Callback = function(value)
		realisticMaxAmount = math.clamp(math.floor(tonumber(value) or 10), 1, MAX_WEAPON_AMOUNT)
	end,
})

local SpawnSelectedRealisticBtn = SpawnerFrame:CreateButton({
	Name = "Spawn Selected",
	Callback = function()
		local minAmount, maxAmount = realisticMinAmount, realisticMaxAmount
		...
		for _, name in ipairs(selectedRealisticNames) do
			local weapon = spawnerWeaponByName[name]
			if weapon then
				selectedCount += 1
		...
		if selectedCount < 1 then
			SpawnSelectedRealisticBtn:Set("Select Weapons First")
		else
			SpawnSelectedRealisticBtn:Set("Spawned " .. spawnedCount .. " Types")
		end

		task.delay(2, function()
			SpawnSelectedRealisticBtn:Set("Spawn Selected")
		end)
	end,
})
...
TradeFrame:CreateSection("Fake Trade")

TradeFrame:CreateButton({
	Name = "Start Trade",
	Callback = function()
		_G.CartiHubPendingFakeTradeUsername = nil
		_G.CartiHubActiveFakeTradeUsername = nil
...
TradeFrame:CreateSection("Offer Spawner")

local selectedOfferName = offerWeaponNames[1]

TradeFrame:CreateInput({
	Name = "Search",
	PlaceholderText = "Search offer items...",
	RemoveTextAfterFocusLost = false,
	Callback = function(query)
		local filtered = filterNames(offerWeaponNames, query)
		OfferDropdown:Refresh(filtered, true)
		selectedOfferName = filtered[1]
	end,
})

OfferDropdown = TradeFrame:CreateDropdown({
	Name = "Offer Item",
	Options = offerWeaponNames,
	CurrentOption = { offerWeaponNames[1] },
	MultipleOptions = false,
	Callback = function(value)
		selectedOfferName = type(value) == "table" and value[1] or value
	end,
})
...
TradeFrame:CreateButton({
	Name = "Add To Offer",
	Callback = function()
		local weapon = selectedOfferName and offerWeaponByName[selectedOfferName]
		if weapon and addSpecificItemToTheirOffer then
			addSpecificItemToTheirOffer(weapon.itemId, weapon.itemType)
		end
	end,
})

TradeFrame:CreateButton({
	Name = "Delete Last Offer Slot",
	Callback = function()
...
TradeFrame:CreateButton({
	Name = "Add Random Their Godly",
	Callback = addRandomGodlyToTheirOffer,
})

TradeFrame:CreateButton({
	Name = "Remove Last Their Item",
	Callback = removeLastTheirOffer,
})

-- Upgrading tab

UpgradingFrame:CreateSection("Fake Trade Upgrade")

UpgradePlayerDropdown = UpgradingFrame:CreateDropdown({
	Name = "Select Player",
	Options = { "None" },
	CurrentOption = { "None" },
	MultipleOptions = false,
	Callback = function(value)
		...
		if name and name ~= "None" then
			CartiHubSetSelectedUpgradePlayer(name)
		else
			CartiHubSetSelectedUpgradePlayer(nil)
		end
	end,
...
local LaunchUpgradeTradeBtn = UpgradingFrame:CreateButton({
	Name = "Launch Fake Trade",
	Callback = function()
		if not CartiHubSelectedUpgradePlayerName then
			LaunchUpgradeTradeBtn:Set("Select A Player First")
			task.delay(2, function()
				LaunchUpgradeTradeBtn:Set("Launch Fake Trade")
			end)
			return
		end
...
AutoSelectLastTradedToggle = UpgradingFrame:CreateToggle({
	Name = "Auto Select Last Traded",
	CurrentValue = false,
	Flag = "CartiHubAutoSelectLastTraded",
	Callback = function(value)
		CartiHubSetAutoSelectLastTraded(value)
	end,
})

-- Settings tab

SettingsFrame:CreateSection("Trade")

SettingsFrame:CreateToggle({
	Name = "Trade Request",
	CurrentValue = false,
	Flag = "CartiHubTradeRequest",
...
SettingsFrame:CreateToggle({
	Name = "Auto Trade",
	CurrentValue = false,
	Flag = "CartiHubAutoTrade",
	Callback = function() end,
})

SettingsFrame:CreateSection("Friend Join")

SettingsFrame:CreateSlider({
	Name = "Friend Joined Delay (Sec)",
	Range = { 30, 300 },
	Increment = 10,
	CurrentValue = 30,
	Callback = function(value)
		friendJoinDelaySeconds = value
	end,
})

SettingsFrame:CreateButton({
	Name = "Friend Joined",
	Callback = function()
		showRandomFakeFriendJoinNotification()
	end,
...
AutoFriendJoinToggle = SettingsFrame:CreateToggle({
	Name = "Auto Friend Join",
	CurrentValue = false,
	Flag = "CartiHubAutoFriendJoin",
	Callback = function(value)
		setAutoFriendJoinEnabled(value)
	end,
})

SettingsFrame:CreateSection("Avatar Changer")

local avatarUserIdText = ""

SettingsFrame:CreateInput({
	Name = "Username or User ID",
	PlaceholderText = "Username or User ID",
	RemoveTextAfterFocusLost = false,
	Callback = function(value)
		avatarUserIdText = value
	end,
})

local ChangeAvatarBtn = SettingsFrame:CreateButton({
	Name = "Change Avatar",
	Callback = function()
		local ok, errText = applyAvatarFromUserId(avatarUserIdText)
		ChangeAvatarBtn:Set(ok and "Avatar Changed" or tostring(errText or "Failed"))
		task.delay(2, function()
			ChangeAvatarBtn:Set("Change Avatar")
		end)
	end,
})

local ResetAvatarBtn = SettingsFrame:CreateButton({
	Name = "Reset Avatar",
	Callback = function()
		local ok, errText = applyAvatarFromUserId(localPlayer.UserId)
		ResetAvatarBtn:Set(ok and "Avatar Reset" or tostring(errText or "Failed"))
		task.delay(2, function()
			ResetAvatarBtn:Set("Reset Avatar")
		end)
	end,
})

-- Keys tab (informational only, mirrors the fixed hotkeys wired further below)

KeybindsFrame:CreateSection("Keybinds")
KeybindsFrame:CreateLabel("Toggle UI: None")
KeybindsFrame:CreateLabel("Start Trade: Z")
KeybindsFrame:CreateLabel("Friend Join: F")
KeybindsFrame:CreateLabel("Spawn All: None")

-- _G.CartiHub* compatibility shims (external code may call these by name)

function populateWeaponSpawner()
	spawnerWeaponList = collectSpawnerWeapons(function(_, _, data)
		return isSpawnerRarity(data)
	end)
	spawnerWeaponByName = {}
	spawnerWeaponNames = {}
	...
	if WeaponBrowserDropdown then
		WeaponBrowserDropdown:Refresh(spawnerWeaponNames, true)
	end

	if RealisticDropdown then
		RealisticDropdown:Refresh(spawnerWeaponNames, true)
	end
end

function populateOfferSpawner()
	offerWeaponList = collectSpawnerWeapons(function(itemId, itemType, data)
		return isOfferSpawnerWeapon(data) and canFakeTradeItem(itemId, itemType)
	end)
	offerWeaponByName = {}
	offerWeaponNames = {}
	...
	if OfferDropdown then
		OfferDropdown:Refresh(offerWeaponNames, true)
	end
end

populateRealisticSpawner = populateWeaponSpawner

function AddWeaponBox(itemId, itemType, weaponName, rarity)
	local name = weaponName or itemId
	if not spawnerWeaponByName[name] then
		spawnerWeaponByName[name] = { itemId = itemId, itemType = itemType, name = name, rarity = rarity }
		table.insert(spawnerWeaponNames, name)
...
_G.CartiHubStartTradeRequest = function()
	_G.CartiHubPendingFakeTradeUsername = nil
	_G.CartiHubActiveFakeTradeUsername = nil
	showIncomingTradeRequest()
end
...
_G.CartiHubSetAcceptDebug = function(enabled)
	_G.CartiHubDebugFakeTradeAccept = enabled == true
	_G.CartiHubLastAcceptTrace = nil
	CartiHubTraceFakeAcceptState("manual_toggle")
end
_G.CartiHubDumpAcceptState = function()
	_G.CartiHubLastAcceptTrace = nil
	CartiHubTraceFakeAcceptState("manual_dump")
end
...
print("[Carti Hub] Weapon Browser and Realistic Spawner list Godly and Ancient weapons only.")
